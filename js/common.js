...
var utilities = {
	...
	words: {
		...
		alphanum: "ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890".split(""),
		syllables: {
		...
		getRandomID: function(len) {
			...
			var s = "";
			for (var i = 0; i < len; i++) {
				s += getRandom(utilities.words.alphanum);
			}
			return s;
		},
...
//=================================================================
//=================================================================
KColor.prototype.toSVGRGB = function(shade, fade) {
	if (shade !== undefined) {
		var css;
		this.use(function(h, s, b, a) {
...
function indexOrEndOf(s, query, startFrom) {
	var index = s.indexOf(query, startFrom);
	if (index < 0)
		return s.length;
	return index;
}

function scrollToTarget(holder, target) {
	var y = target.offset().top + holder.scrollTop();
	y -= 50;
	//y = 30;
...
function createStorableDropdown(holder, id, label, options, onChange, numeric) {
	var div = $("<div/>").appendTo(holder);
	var label = $("<span/>", {
		html: label + " "
	}).appendTo(div);
	...
	function setTo(v) {
		...
		if (numeric)
			v = parseFloat(v);
		console.log(id + ": " + v);
		localStorage.setItem("stored-" + id, v);
		onChange(v);

	}

	// On load
	var v = localStorage.getItem("stored-" + id, v);
	setTo(v);
	select.val(v);
}